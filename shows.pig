...
data = LOAD '/user/maria_dev/pig/Netflix_Shows.csv' USING PigStorage(',') AS (
    show_id:int, 
    type:chararray, 
    title:chararray, 
    director:chararray, 
...
processed_data = FOREACH filtered_data GENERATE 
    show_id, 
    type, 
    title, 
    director, 
...
data_country1 = LOAD '/user/maria_dev/pig/Netflix_Shows_Country1.csv' USING PigStorage(',') AS (
    show_id:int, 
    type:chararray, 
    title:chararray, 
    director:chararray, 
...
data_country2 = LOAD '/user/maria_dev/pig/Netflix_Shows_Country2.csv' USING PigStorage(',') AS (
    show_id:int, 
    type:chararray, 
    title:chararray, 
    director:chararray, 
...
-- Check the loaded country-specific data
DUMP data_country1;
DUMP data_country2;
...
-- Optional: Export data for further statistical analysis
